...
This is the executable plan for the next agent. The current agent stopped implementation at the user's request, installed the latest APK, and reviewed the app on the TCL. **The title, compact-search and prominent-Review changes specified below are planned, not implemented.** Complete them when the user starts the next implementation task. Do not interrupt the seven-day trial with unsolicited changes or repeated testing.
...
- **Shelf:** reduce the visual competition between Continue reading, bold titles, side markers and full-width progress strokes. Keep Continue reading a compact single entry; source and remaining time should aid choosing the next read without becoming another large card. Use unread/in-progress/finished/link-only state consistently; a colored line alone must not imply an undocumented category.
...
- [ ] **Native final-end fix:** `NativeArticleView.isAtEnd()` now examines the last non-whitespace text line. This last adjustment is in installed `b14f128`; compile/lint passed, but its final device recheck was stopped by the user. On one short QA article, scroll naturally to the end, confirm the quiet Finish card appears and the viewport stays stable, then scroll back. Reaching the end must not write completion credit. Test menu Finish & archive → Undo only if the new check reveals a regression; that explicit action previously passed.
- [ ] **Native selection after continuity fix:** press-and-hold once, adjust one handle, invoke Highlight and ensure one saved selection, not duplicates. Earlier real native selection and Undo passed, but the final idle-cursor clearing change needs this small follow-up. Preserve Copy, Share, system actions, 8dp inset, autoscroll and bounded parts.
- [ ] **Position:** after any reader changes, test one middle passage through appearance/bold and Back/reopen; test one Speed return only if that code path changes. Previous targeted evidence showed scrolling, reopen and Speed return retaining the passage after clearing an idle offset-zero cursor. Do not reintroduce `TextView` automatic cursor scrolling or nested transition launches.
- [ ] **One enlarged-text pass:** font scale 1.4 on QA; title/search/four shelves/appearance/filter/Review actions reachable. Record and restore the original system value in a finally block. Not an exhaustive accessibility matrix.
- [ ] **Search UI combinations:** keyboard visibility was observed in the latest handover review, but multiword autocomplete, >50 paging UI and all active-constraint combinations were not fully exercised. Cover the small package-B checklist, not every query.
