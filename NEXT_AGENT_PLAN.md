...
This is the executable plan for the next agent. **Work packages A–D below are now implemented, committed locally as `99c5cf9` on `codex/reliability-finalize`, and installed in place as the normal `com.reader.app` debug candidate (APK SHA-256 `a7cc78a9187a1a59fa027d887be91ae8c4c72e24933940981d1ddce69c978736`).** Section 9 records the two previously-missing native checks, which now have TCL evidence. Do not interrupt the seven-day trial with unsolicited changes or repeated testing; when the user reports a trial issue, reproduce that exact path, make the smallest complete fix and repeat only the affected checks.
