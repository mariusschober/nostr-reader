...
- E-ink theme (`ui/theme/Tokens.kt`, `ui/theme/Motion.kt`, `prefs/Prefs.kt`): does `ThemeMode.EINK` decode from storage without renaming existing values, and is the theme applied from the stored mode rather than inferred? Does a nested `ReaderTheme` keep the monochrome/reduced-motion override? Does leaving E-ink restore the exact saved reading background/font, or can the policy leak? Is reduced motion limited to decorative transitions (chrome fade, settle, reveal, the native swipe settle) while drag physics, direct touch scrolling and native selection stay untouched? Are the e-ink colours genuinely monochrome with non-colour state for Important and errors, and does the selected-container inversion hold for every chip/toggle?
...
- E-ink: is the whole app genuinely monochrome - destination header, article, highlight cards, chips, dialogs, sheets, Review, Speed, search fields and system bars - with readable contrast, and does the Appearance sheet correctly say the app-wide theme owns the background while E-ink is active?
...
- E-ink: does switching to E-ink preserve the reading position, and does leaving it restore the previous appearance without resetting preferences? Is any motion left that the theme should suppress, or any functional motion (drag physics, native selection) wrongly suppressed?
