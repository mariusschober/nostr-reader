...
Updated 2026-09-10 for the user-approved focused implementation, then again after the shared-header, compact-search and prominent-Review pass, and again on 2026-09-11 for the final usability pass (Reader rename, four reading actions, once-only highlight help, warmer #FFF1E5 Paper surface) and the quote-tap Review pass (full quote cards, Review from a tapped quote without losing the round). Superseded rows are replaced rather than kept. UI copy and its assertions change together.
...
| Quote card | Source title · Open source · More · Important badge | The full quote is shown on the card with adaptive type — base +6sp up to 160 visible characters (grapheme clusters), base +3sp up to 450, otherwise base, never below the reader's chosen size. No six-line preview and no "Read full highlight". The 24dp Important star sits over the upper border and announces "Important highlight". Attribution is subordinate; Open source and More live in the compact footer and do not trigger the card tap. Missing sources retain quote and attribution. |
| Highlights | Newest · Important · Shuffle | Newest default; local quote/source-title search. Shuffle shows its own selected state, so it never reads as an unselected action. Changing order never advances Review, and feed filters do not redefine the persisted round. |
| Review from a quote | Revisit your saved passages. · Continue · Review again | Tapping a highlight card presents that quote for Review without discarding an unfinished round: the round's cursor, queue, phase and Important-bonus history are preserved, the focused quote is reviewed once on Next, and leaving before Next keeps it for Continue. Entering Review records nothing, and a filtered result starts at that quote then continues the whole-highlights round. |
| Review entry | Review highlights · Start a round · Continue · Review again | A floating banner above the bottom navigation (≥64dp, 16dp side margins), not a top card. Visible on arrival and at the top, hidden after ~16dp of deliberate downward scrolling and shown again on upward scrolling; hidden during keyboard entry, multi-selection, modal actions and active Undo. It is an overlay, so showing or hiding it never re-layouts the list, and the list reserves bottom room for the last footer. It reads persisted state only — it never starts, advances or repairs a cycle just to render. With no highlights the actionable empty state replaces it. |
