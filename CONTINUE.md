# Continue from the trial-fix completion checkpoint — 14 September 2026

**Pushing and continuation (14 September 2026, later):** the branch is pushed to `origin/codex/reliability-finalize` at the owner's request (no release). One required outcome remains open: the native selection lingers after a settled pen highlight. Read [NEXT_AGENT_PROMPT_TRIAL_FIXES_CONTINUATION_20260914.md](NEXT_AGENT_PROMPT_TRIAL_FIXES_CONTINUATION_20260914.md) for the diagnosis, the planned minimal fix and the focused verification protocol; the rest of this file is the earlier state.

**Current state (14 September, TCL pass):** the frozen source is **`0ca22ac`**, which repairs the Back-during-gesture deviation the S23 walkthrough found; the `d58067f` paragraph below is superseded on that point. Host gates re-ran green at `0ca22ac` (`lintDebug` 0 errors, 268 unit tests, normal and QA `assembleDebug`, 16 KiB alignment). Normal `com.reader.app` (SHA-256 `003b4863a61aec09d7d4fbc2b4fad82cece9d224ebc89b180834cd77b7d2b0de`) is installed in place on the TCL `ZXKRS4VKGQ8PWGEQ`; the pulled-back APK matches the archive, all eight Room tables are byte-identical, the key-file digest is unchanged, and preferences changed only by the one-time `displayMode = STANDARD` materialization. The TCL NXTPAPER QA package confirmed monochrome dark and light, the compact pen dock, a settled dark highlight, and fullscreen focus in dark monochrome with no light outer border. The pen-cancel repair was verified on device (Back mid-gesture saved nothing; the same gesture without Back saved one row). Read [IMPLEMENTATION_HANDOFF_20260914.md](IMPLEMENTATION_HANDOFF_20260914.md) and [artifacts/trial-fixes-20260914/EVIDENCE.md](artifacts/trial-fixes-20260914/EVIDENCE.md) for the exact artifacts, hashes and measured results. Two non-blocking deviations remain: a cosmetic native-selection remnant after settlement, and the Appearance/Done header scrolling away at full expansion. Do not clear or uninstall the owner package; `docs/COPY_DECK.md` is unchanged because the repair altered behaviour only.

**Current state (14 September):** the trial-fix pass is implemented and frozen at **`d58067f`** on `codex/reliability-finalize`. The brief ([NEXT_AGENT_PROMPT_DEEPSEEK_TRIAL_FIXES_20260914.md](NEXT_AGENT_PROMPT_DEEPSEEK_TRIAL_FIXES_20260914.md)) is fully implemented — packages A–E plus a lint fix. Host gates pass: `lintDebug` 0 errors, 268 unit tests, normal and QA `assembleDebug`, and the 16 KiB native-alignment check. Normal `com.reader.app` (SHA-256 `7ff086955e9fc4bc63e3668d6a293818cbea910c8f90ee93a651119fb108e61f`) is installed in place on the S23 Ultra `R3CW404GVBL`; all eight Room tables are byte-identical before and after, and only the one-time `displayMode = STANDARD` materialization changed preferences. The new Display control and both monochrome variants were confirmed on the S23. The device walkthrough was then completed there on the QA package (see the handoff): continuous highlighting with one settled row per gesture, Undo, short-drag scrolling and cancellation saving nothing; ordinary native selection with handles and actions; article highlights and article-scoped Review; label colours with the compact `#label` library badge; service-owned background listening through Home with the now-playing row and Stop; and the partially-open Appearance sheet with every control reachable and the reading passage preserved. Two deviations are recorded there: Back pressed while a pen highlight drag is still held does not cancel it, and a cancelled gesture leaves a cosmetic selection remnant. A focused monochrome-dark view was not captured. **The TCL `ZXKRS4VKGQ8PWGEQ` was not connected**, so the TCL installation hash and the NXTPAPER physical-appearance checks are BLOCKED and are the next input.

Read [IMPLEMENTATION_HANDOFF_20260914.md](IMPLEMENTATION_HANDOFF_20260914.md) and [artifacts/trial-fixes-20260914/EVIDENCE.md](artifacts/trial-fixes-20260914/EVIDENCE.md) for exact artifacts, hashes, screenshots and measured PASS/BLOCKED results. Do not clear or uninstall the owner package, and do not repeat historical intake or broad test campaigns.

## Historical checkpoint — focused repairs and TCL trial (11 September 2026)

Read **[IMPLEMENTATION_HANDOFF_20260911.md](IMPLEMENTATION_HANDOFF_20260911.md)** and **[evidence/repairs-20260911/manifest.json](evidence/repairs-20260911/manifest.json)** for the previous installed checkpoint. Product source is complete through `75b369b`; `eca253a` is test-only and `b0c360a` finalized the handoff. Normal `com.reader.app` was installed on TCL `ZXKRS4VKGQ8PWGEQ`, with local/installed APK SHA-256 `d3bdcdae7dc1dae525d5b04805fe31aaf331eb4cb98420df8163dd8278ea0236`. Owner summaries and the on-device key digest matched before and after the install; the app was launched once after those comparisons. The focused views left **NOT MEASURED** were Green/double wrapped E-ink rendering, Speed in normal/E-ink, and one enlarged-system-text layout spot check. S23 geometry, completion/Undo, ordinary wrapped monochrome highlighting and Review source/Back were recorded as PASS. Do not clear or uninstall the owner package or repeat intake/broad testing.
