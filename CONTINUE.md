...
- Checks: 247 Android unit tests pass (incl. 3 new count cases + coach materialization case), `lintDebug` passes, 10/10 Chrome extraction tests pass, typecheck + build pass, `HighlightSessionInstrumentedTest` passes 1/1 on TCL T807D with APKs left installed, all 12 native `.so` entries 16KB page-aligned.
